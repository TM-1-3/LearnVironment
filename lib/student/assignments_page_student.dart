...
class AssignmentsPageStudent extends StatefulWidget {
  final String id;
  const AssignmentsPageStudent({super.key, required this.id});

  @override
  AssignmentsPageStudentState createState() => AssignmentsPageStudentState();
}

class AssignmentsPageStudentState extends State<AssignmentsPageStudent> {
  String _searchQuery = "";
    ...
    try {
      ...
      if (assignmentData != null && userId.isNotEmpty && mounted) {
        Navigator.push(context,
          MaterialPageRoute(
            builder: (context) => AssignmentPageStudent(assignmentData: assignmentData),
          ),
        );
      }
    } catch (e) {
